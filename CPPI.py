...
def getParameters(test_num=1, rate_type=0, trading_year=1, trading_day_per_year=244, rf=0.0015,
    ...
    # 定义市场参数
    parameters["trading_year"] = 2
    parameters["rf"] = 0.015  # 无风险利率
    parameters["trading_day_per_year"] = 244
    parameters["rf_daily"] = rf / trading_day_per_year
    parameters["trading_day_sum"] = trading_year * trading_day_per_year
    parameters["init_nav"] = 1000000  # 初始本金
    parameters["adj_period"] = 5  # 调整周期
    parameters["guarantee_rate"] = 0.8  # 保本比例
    parameters["risk_multipler"] = 2  # 风险乘数
    parameters["risk_trading_fee_rate"] = 0.006  # 风险资产交易费率
    return parameters
    ...
    for i in range(len(data)):
        ...
        for t in range(2, p["trading_day_sum"] + 1):
            ...
            rf_asset[t, risk_asset[t, :] <= 0] = nav[t, risk_asset[t, :] <=
                                                     0] - risk_asset[t, risk_asset[t, :] <= 0] * p[
